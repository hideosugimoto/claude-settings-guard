...
import {
  ...
  isLegacySyntax,
  isValidToolName,
} from '../src/core/pattern-parser.js'

describe('pattern-parser: parsePattern', () => {
  ...
  it('returns fallback for unknown patterns', () => {
    const result = parsePattern('SomeUnknownThing', 'allow')
    expect(result.toolName).toBe('SomeUnknownThing')
    expect(result.isLegacy).toBe(false)
  })
})

describe('pattern-parser: isLegacySyntax', () => {
...
describe('pattern-parser: isValidToolName', () => {
