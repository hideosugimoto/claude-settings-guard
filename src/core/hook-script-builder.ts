...
export function generateNonBashToolCheck(toolName: string, rules: readonly DenyRule[]): string {
  ...
  if (toolName === 'Grep') {
    ...
    lines.push(`  # Normalize: prepend ./ so deny regex patterns (e.g. .*/.env$) can match bare filenames`)
    lines.push(`  if [ -n "$grep_glob" ] && [[ "$grep_glob" != /* ]]; then`)
    lines.push(`    grep_glob="./$grep_glob"`)
    lines.push(`  fi`)
